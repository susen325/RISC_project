...
module top_core (
    ...
    // =========================================================
    // 3. REGISTER FILE & ALIAS TABLE (RAT)
    // =========================================================
    reg [31:0] reg_file [0:31];
    reg [3:0]  rat      [0:31];
    
    // Read Operands for Dispatch
    wire [31:0] vj_raw = (rs1 == 0) ? 32'b0 : reg_file[rs1];
    wire [31:0] vk_raw = (rs2 == 0) ? 32'b0 : reg_file[rs2];
    wire [3:0]  qj_raw = (rs1 == 0) ? 4'b0  : rat[rs1];
    wire [3:0]  qk_raw = (rs2 == 0) ? 4'b0  : rat[rs2];
    ...
    dynamic_execute DISPATCHER (
        ...
        .rs_alu_busy(rs_alu_busy), .rs_mul_busy(1'b0), .rs_div_busy(1'b0), .rs_mem_busy(rs_mem_busy),
    ...
    cdb_arbiter ARBITER (
        ...
        .mul_req(1'b0), .mul_tag(4'b0), .mul_value(32'b0), // Dummy for now
        .div_req(1'b0), .div_tag(4'b0), .div_value(32'b0), // Dummy for now
        .mem_grant(mem_grant), .alu_grant(alu_grant), 
        .cdb_valid(cdb_valid), .cdb_tag(cdb_tag), .cdb_value(cdb_value),
    ...
    rob #( .ENTRIES(8) ) ROB_UNIT (
        ...
        .issue_is_store(issue_is_store), .issue_is_branch(issue_is_branch),
