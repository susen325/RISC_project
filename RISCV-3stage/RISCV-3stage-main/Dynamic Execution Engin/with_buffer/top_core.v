...
module top_core (
    ...
    wire        pipeline_stall;
    wire        commit_mem_we; // Declared once for RS_MEM and ROB
    ...
    // =========================================================
    wire commit_reg_we; // Restored this wire!
   ...
   reg [15:0] led_reg;
    always @(posedge clk or negedge reset) begin
        ...
        // THE FIX: Remove the "x4" filter. 
        // Now, whenever ANY register (except x0) gets written to, 
        // instantly flash that new value to the LEDs!
        else if (commit_reg_we && commit_reg_rd != 5'd0) begin
